  ...
  function requireAuth(){ 
    // Se não tem token, só redireciona se NÃO estiver no index.html
    if(!getToken()){
  ...
  document.getElementById("loginForm")?.addEventListener("submit", async (e)=>{
    ...
    try{
      ...
      if(!res.ok){ 
        document.getElementById("msg").textContent = data.message || "Erro"; 
        return; 
      }
  ...
  document.getElementById("registerForm")?.addEventListener("submit", async (e)=>{
    ...
    try{
      ...
      const data = await res.json();
      document.getElementById("msg").textContent = data.message || "Registro ok";
  ...
  // ---------- run charts ONLY where charts exist ----------
